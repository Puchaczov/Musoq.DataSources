...
public class KubernetesLibrary : LibraryBase
{
    ...
    {
        var plainTextBytes = Encoding.GetEncoding(encoding).GetBytes(text);
        return Convert.ToBase64String(plainTextBytes);
    }
    ...
    [BindableMethod]
    public string? GetLabelOrDefault([InjectSpecificSource(typeof(IWithObjectMetadata))] IWithObjectMetadata row,
        string key)
    {
