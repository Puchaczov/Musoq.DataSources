...
{
    private const string TwoPatternRequest =
        "{\"version\":1,\"patterns\":[" +
        "{\"id\":\"todo\",\"pattern\":\"TODO\",\"mode\":\"literal\"}," +
        "{\"id\":\"fixme\",\"pattern\":\"FIXME\",\"mode\":\"literal\"}]}";

    private const string MissingPatternRequest =
        "{\"version\":1,\"patterns\":[" +
        "{\"id\":\"todo\",\"pattern\":\"TODO\",\"mode\":\"literal\"}," +
        "{\"id\":\"missing\",\"pattern\":\"MISSING\",\"mode\":\"literal\"}]}";
    ...
    public void CompiledManyQuery_ShouldExposeTypedLabeledOccurrences()
    {
        ...
        try
        {
            ...
            var result = Compile(
                    $"select Path, PatternId, MatchIndex from search.many('{escapedRoot}', '{TwoPatternRequest}') " +
    ...
    public void CompiledSql_ShouldProduceTypedPerPatternCountsAndAnyHits()
    {
        ...
        try
        {
            ...
            var result = Compile(
                    ...
                    $"from search.many('{escapedRoot}', '{TwoPatternRequest}') m " +
    ...
    public void CompiledSql_ShouldExpressNoneAndMissingPatternWithoutFalseNegatives()
    {
        ...
        try
        {
            ...
            var none = Compile(
                    ...
                    $"left outer join search.many('{escapedRoot}', '{TwoPatternRequest}') m on p.Path = m.Path " +
                    "where m.Path is null order by p.Path")
                .Run();
            var allWithMissing = Compile(
                    $"select m.Path from search.many('{escapedRoot}', '{MissingPatternRequest}') m " +
                    "group by m.Path having Count(distinct m.PatternId) = 2 order by m.Path")
                .Run();
    ...
    [TestMethod]
    public void ManySource_ShouldRejectRegexBeforeOpeningContent()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), $"musoq-search-many-missing-{Guid.NewGuid():N}");
