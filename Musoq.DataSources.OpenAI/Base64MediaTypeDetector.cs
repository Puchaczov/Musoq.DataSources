...
internal class Base64MediaTypeDetector
{
    public static string DetectMimeType(string base64String)
    {
        ...
        if (bytes.Length >= 2)
        {
            ...
            if (bytes is [0x25, 0x50, 0x44, 0x46, ..])
            {
                return "application/pdf";
            }
            
            if (bytes is [0x52, 0x49, 0x46, 0x46, ..])
            {
                return "image/webp";
            }
        }
        
        // Check for SVG (look for XML header and svg tag)
        var decodedStart = System.Text.Encoding.UTF8.GetString(bytes.AsSpan(0, Math.Min(bytes.Length, 256)).ToArray());
