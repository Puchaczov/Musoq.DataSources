...
public class OpenAiLibrary : LibraryBase, ILargeLanguageModelFunctions<OpenAiEntity>
{
    ...
    [BindableMethod]
    public bool IsContentAbout([InjectSpecificSource(typeof(OpenAiEntity))] OpenAiEntity entity, string content,
        string question)
    {
        var api = entity.Api;
    ...
    [BindableMethod]
    public string Sentiment([InjectSpecificSource(typeof(OpenAiEntity))] OpenAiEntity entity, string content,
    ...
    [BindableMethod]
    public string TranslateContent([InjectSpecificSource(typeof(OpenAiEntity))] OpenAiEntity entity, string content,
        string from, string to)
    {
        var api = entity.Api;
        var translateResultTask = DoAsynchronously(() => api.GetCompletionAsync(entity, new List<ChatMessage>
        {
            ChatMessage.CreateSystemMessage(TranslateGivenTextPrompt.Replace("{SOURCE_LANGUAGE}", from)
                .Replace("{DESTINATION_LANGUAGE}", to)),
            ChatMessage.CreateUserMessage(content)
        }));
    ...
    [BindableMethod]
    public string[] Entities([InjectSpecificSource(typeof(OpenAiEntity))] OpenAiEntity entity, string content,
    ...
    [BindableMethod]
    public string LlmPerform<TColumnType>([InjectSpecificSource(typeof(OpenAiEntity))] OpenAiEntity entity,
    ...
    public string DescribeImage([InjectSpecificSource(typeof(OpenAiEntity))] OpenAiEntity entity, string imageBase64)
    {
        ...
        var describeResultTask = DoAsynchronously(() =>
        {
            ...
            return api.GetCompletionAsync(entity, new List<ChatMessage>
            {
                ...
                ChatMessage.CreateUserMessage(ChatMessageContentPart.CreateImagePart(binaryData,
                    Base64MediaTypeDetector.DetectMimeType(imageBase64)))
            });
        });
    ...
    [BindableMethod]
    public string AskImage([InjectSpecificSource(typeof(OpenAiEntity))] OpenAiEntity entity, string question,
        string imageBase64)
    {
        var api = entity.Api;
        var askResultTask = DoAsynchronously(() =>
        {
            const string youAreImageQuestionerAskQuestionAboutImage =
    ...
    [BindableMethod]
    public bool IsQuestionApplicableToImage([InjectSpecificSource(typeof(OpenAiEntity))] OpenAiEntity entity,
        string question, string imageBase64)
    {
        var api = entity.Api;
        var askResultTask = DoAsynchronously(() =>
        {
            var youAreImageQuestionerAskQuestionAboutImage =
                $"You are image based question answerer. Return only answer for the following question: {question}. You must respond with json {{ result: bool }}. Do not comment or explain anything.";
            var binaryData = BinaryData.FromBytes(FromBase64(imageBase64));
            return api.GetCompletionAsync(entity, new List<ChatMessage>
            {
                ChatMessage.CreateSystemMessage(youAreImageQuestionerAskQuestionAboutImage),
                ChatMessage.CreateUserMessage(
                    ChatMessageContentPart.CreateImagePart(binaryData,
                        Base64MediaTypeDetector.DetectMimeType(imageBase64)))
            });
        });
    ...
    {
        [JsonPropertyName("entities")] public required string[] Entities { get; init; }
    }
}