...
internal static class SystemSourcePlanner
{
    public static SourcePlanResult Plan(string name, SourcePlanRequest request)
    {
        ...
        var (acceptedOrderBy, residualOrderBy) = SplitNaturalAscendingOrder(request.OrderBy, nameof(RangeItemEntity.Value));
        ...
        return new SourcePlanResult
        {
            ...
            Cardinality = CardinalityEstimate.Unknown("Range cardinality depends on requested bounds and accepted filters."),
            Diagnostics = [],
            ContractDiagnostics = []
        };
    }

    public static bool Matches(SourcePredicateExpression? predicate, RangeItemEntity entity)
    {
        return predicate switch
        {
            ...
            SourcePredicateComparison comparison => EvaluateComparison(comparison, entity.Value),
            _ => true
        };
    }
    ...
    private static bool IsSupportedPredicate(SourcePredicateExpression expression)
    {
        return expression is SourcePredicateComparison comparison &&
               ...
               columnName.Equals(nameof(RangeItemEntity.Value), StringComparison.OrdinalIgnoreCase) &&
               TryGetInt64(literal.Value, out _);
    }

    private static bool EvaluateComparison(SourcePredicateComparison comparison, long value)
    {
        if (!TryGetComparisonParts(comparison, out _, out var literal, out var op) ||
            !TryGetInt64(literal.Value, out var expected))
            return true;
