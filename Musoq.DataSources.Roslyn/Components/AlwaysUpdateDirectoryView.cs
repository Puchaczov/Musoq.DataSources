    ...
    where TDestinationValue : class
{
    ...
    private async Task SynchronizeAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
            await Parallel.ForEachAsync(_synchronizationQueue.GetConsumingEnumerable(cancellationToken),
    ...
    private async Task StoreWithinDirectoryAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
            await Parallel.ForEachAsync(_itemsToStore.GetConsumingEnumerable(cancellationToken), cancellationToken,
                (item, token) =>
                {
    ...
    private void ScanDirectory(object? state)
    {
        ...
        try
        {
            ...
            foreach (var path in cachedKeys.Keys.Select(cachedKey => IFileSystem.Combine(_directoryPath, cachedKey))
                         .Where(path => !_fileSystem.IsFileExists(path))) _synchronizationQueue.Add(new FileInfo(path));
        }
        catch (Exception ex)
        {
