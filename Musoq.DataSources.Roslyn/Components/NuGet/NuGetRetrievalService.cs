...
internal sealed class NuGetRetrievalService(
    INuGetPropertiesResolver? nuGetPropertiesResolver,
    IFileSystem? fileSystem,
    IHttpClient? httpClient) : INuGetRetrievalService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var (xmlDoc, namespaceManager) =
                await CreateXmlDocumentAndNamespaceManager(nuspecFilePath, fileSystem, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var strategies = ResolveWebScrapeStrategies(httpClient, commonResources, nuGetPropertiesResolver,
                cancellationToken);
    ...
    {
        var requestUrlBase =
            $"{apiEndpoint}?packageName={commonResources.PackageName}&packageVersion={commonResources.PackageVersion}&propertyName={propertyName}";
    ...
    public async Task<string[]> GetPackageVersionsAsync(string packageName, CancellationToken cancellationToken)
    {
        ...
        if (versionsResponse is null) return [];

        if (versionsResponse.Versions is null) return [];

        return versionsResponse.Versions;
    }

    public async Task<string?> DownloadPackageAsync(string packageName, string packageVersion, string packagePath,
        CancellationToken cancellationToken)
    {
        var response = await httpClient.GetAsync($"https://www.nuget.org/api/v2/package/{packageName}/{packageVersion}",
            cancellationToken);
    ...
    private static IReadOnlyDictionary<string, Func<XmlDocument, XmlNamespaceManager, Task<string?>>>
        ResolveNuspecStrategies(NuGetResource? commonResources, INuGetPropertiesResolver? nuGetPropertiesResolver,
            CancellationToken cancellationToken)
    {
        return new Dictionary<string, Func<XmlDocument, XmlNamespaceManager, Task<string?>>>
        {
            ...
            ["LicensesNames"] = (doc, manager) =>
                NuspecHelpers.GetLicensesNamesFromNuspecAsync(doc, manager, commonResources, nuGetPropertiesResolver,
                    cancellationToken),
            [nameof(NuGetLicense.LicenseUrl)] = NuspecHelpers.GetLicenseUrlFromNuspecAsync,
            [nameof(NuGetLicense.LicenseContent)] = (doc, manager) =>
                NuspecHelpers.GetLicenseContentFromNuspecAsync(doc, manager, commonResources, cancellationToken)
        };
    }

    private static IReadOnlyDictionary<string, Func<string, CancellationToken, Task<Func<Task<string?>>>>>
        ResolveWebScrapeStrategies(IHttpClient? client, NuGetResource? commonResources,
            INuGetPropertiesResolver? nuGetPropertiesResolver, CancellationToken cancellationToken)
    {
        var capturedClient = client;
        return new Dictionary<string, Func<string, CancellationToken, Task<Func<Task<string?>>>>>
        {
            [nameof(NuGetResource.ProjectUrl)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            [nameof(NuGetResource.Title)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            [nameof(NuGetResource.Authors)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            [nameof(NuGetResource.Owners)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            [nameof(NuGetResource.RequireLicenseAcceptance)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            [nameof(NuGetResource.Description)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            [nameof(NuGetResource.Summary)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            [nameof(NuGetResource.ReleaseNotes)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            [nameof(NuGetResource.Copyright)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            [nameof(NuGetResource.Language)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            [nameof(NuGetResource.Tags)] = (_, _) =>
                Task.FromResult<Func<Task<string?>>>(() => Task.FromResult<string?>(null)),
            ["LicensesNames"] = async (url, token) =>
                await NugetHelpers.DiscoverLicensesNamesAsync(url, commonResources, capturedClient,
                    nuGetPropertiesResolver, token),
            [nameof(NuGetLicense.LicenseUrl)] = async (url, token) =>
                await NugetHelpers.DiscoverLicenseUrlAsync(url, commonResources, capturedClient, token),
            [nameof(NuGetLicense.LicenseContent)] = async (url, _) =>
                await NugetHelpers.DiscoverLicenseContentAsync(url, commonResources, capturedClient, cancellationToken)
        };
    }

    private static async Task<(XmlDocument, XmlNamespaceManager)> CreateXmlDocumentAndNamespaceManager(
        string nuspecFilePath, IFileSystem? fileSystem, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrEmpty(nugetNamespace)) namespaceManager.AddNamespace("nu", nugetNamespace);
    ...
    {
        [JsonPropertyName("versions")] public required string[]? Versions { get; init; }
    }
}