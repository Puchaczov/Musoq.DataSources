        ...
        private class SpdxTokenizer(string? input)
        {
            ...
            private Token NextToken()
            {
                while (true)
                {
                    ...
                    {
                        var start = _position;
                        while (_position < input.Length && IsIdentifierChar(input[_position]))
                        {
                            _position++;
                        }
                        ...
                        if (value.Equals("WITH", StringComparison.OrdinalIgnoreCase)) return new Token(TokenType.With, value);
