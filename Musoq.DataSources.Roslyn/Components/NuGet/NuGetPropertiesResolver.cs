...
public class NuGetPropertiesResolver(string baseUrl, IHttpClient? httpClient) : INuGetPropertiesResolver
{
    ...
    public async Task<string[]> GetLicensesNamesAsync(string licenseContent, CancellationToken cancellationToken)
    {
        ...
        fileContent.Headers.ContentType = new MediaTypeHeaderValue("application/text");
        ...
        httpRequestMessage.Headers.Add("Musoq-Append-Url-Part-To-Persistent-Cache-Key",
            ComputeLicenseContentMd5(licenseContent));
    ...
    {
        [JsonPropertyName("response")] public required Licenses[] Response { get; init; }
    }

    private class Licenses
    {
        [JsonPropertyName("licenseName")] public required string LicenseName { get; init; }
    }
}