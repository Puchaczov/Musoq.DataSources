    ...
    : CSharpSolutionRowsSourceBase(queryCancelledToken)
{
    ...
    protected override async Task CollectChunksAsync(BlockingCollection<IReadOnlyList<IObjectResolver>> chunkedSource, CancellationToken cancellationToken)
    {
        ...
        logger.LogTrace("Loading solution file using AdHocWorkspace with custom parser: {solutionFilePath}", solutionFilePath);
        
        // Create AdHocWorkspace and use custom solution parser
        var adHocWorkspace = new AdhocWorkspace();
        var solutionParser = new SolutionParser(logger);
        var solution = await solutionParser.ParseSolutionAsync(adHocWorkspace, solutionFilePath, cancellationToken);
        
        // Create the solution entity using the parsed solution
        ...
        logger.LogTrace("Solution initialized with AdHocWorkspace using custom parser.");
        ...
        // Dispose the workspace when done
