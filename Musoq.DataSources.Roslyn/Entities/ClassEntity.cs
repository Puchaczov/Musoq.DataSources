...
public class ClassEntity : TypeEntity
{
    ...
    public double LackOfCohesion
    {
        get
        {
            ...
            for (var j = i + 1; j < methods.Count; j++)
            {
                ...
                var hasIntersection =
                    fieldUsage.Values.Any(fieldSet => fieldSet.Contains(method1) && fieldSet.Contains(method2));
    ...
    public int EfferentCoupling
    {
        get
        {
            ...
            if (Symbol.BaseType != null && Symbol.BaseType.Name != "Object") dependencies.Add(Symbol.BaseType.Name);

            foreach (var iface in Symbol.Interfaces) dependencies.Add(iface.Name);
    ...
    private static HashSet<string> GetUsedFields(MethodDeclarationSyntax method, SemanticModel semanticModel)
    {
        ...
        foreach (var identifier in method.DescendantNodes().OfType<IdentifierNameSyntax>())
        {
            ...
            if (symbol is IFieldSymbol fieldSymbol) usedFields.Add(fieldSymbol.Name);
        }
