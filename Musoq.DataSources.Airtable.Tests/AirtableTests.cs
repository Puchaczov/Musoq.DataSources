    ...
    public void WhenTableRequested_ShouldReturnBases()
    {
        ...
        api.Setup(f => f.GetRecordsChunks(It.IsAny<IReadOnlyCollection<string>>()))
            .Returns(new List<IReadOnlyList<AirtableRecord>>
            {
                new[]
                {
                    CreateAirtableRecord(fields, "\"single line text\"", "\"long text\"", "1", "true", "\"2020-01-01\"",
                        "29", "0.1", "\"test@test.ok\"")
                }
            });

        var query =
            "select Field1, Field2, Field3, Field4, Field5, Field6, Field7, Field8 from #airtable.records('test')";
