...
using Musoq.DataSources.SeparatedValues.Benchmark.Comparisons;

namespace Musoq.DataSources.SeparatedValues.Benchmark.Measurements;

/// <summary>
/// Compares like-for-like legacy, query-scoped struct, and query-scoped class
/// materialization through the SeparatedValues datasource assembly.
/// </summary>
[MemoryDiagnoser]
[DisassemblyDiagnoser(printSource: true, exportCombinedDisassemblyReport: true)]
public class SeparatedValuesQueryScopedSourceMaterializationBenchmarks
{
    ...
    private FrozenByteNativeLegacySchema _legacySchema = null!;
    ...
    public void Setup()
    {
        ...
        _legacySchema = new FrozenByteNativeLegacySchema();
        var queryScopedSchema = new SeparatedValuesSchema();
        _queryScopedSchema = queryScopedSchema;
        _context = CreateExecutionContext(
            queryScopedSchema,
            _path,
            _columns,
            _columns.Select(static column => column.ColumnName).ToArray(),
            sourceContextId: "source-full-string");
        _numericContext = CreateExecutionContext(
            queryScopedSchema,
            _numericPath,
            _numericColumns,
            _numericColumns.Select(static column => column.ColumnName).ToArray(),
            sourceContextId: "source-full-numeric");
        _selectiveContext = CreateExecutionContext(
            queryScopedSchema,
            _path,
            _columns,
            ["Column0"],
            sourceContextId: "source-selective");
        _rejectionContext = CreateExecutionContext(
            ...
            sourceContextId: "source-rejection");
        _earlyTakeContext = CreateExecutionContext(
            ...
            _columns.Select(static column => column.ColumnName).ToArray(),
            take: 16,
            sourceContextId: "source-early-take");
    ...
    public long LegacyObjectArrayMaterialization()
    {
        ...
        for (var row = 0; row < Rows; row++)
        {
            ...
            for (var column = 0; column < FieldCount; column++)
                checksum += ((string?)values[column])?.Length ?? 0;
        }
    ...
    private BenchmarkOutcome RunLegacy(
        string path,
        SourceExecutionContext context,
        int projectedFieldCount)
    {
        var source = _legacySchema.GetRowSource(path, ",", true, 0, context);
        return ConsumeLegacyRows(source, context, projectedFieldCount);
    }

    private static BenchmarkOutcome ConsumeLegacyRows(
        RowSource<object?[]> source,
        SourceExecutionContext context,
        int projectedFieldCount)
    {
        var outcome = new BenchmarkOutcomeBuilder();
    ...
    private static SourceExecutionContext CreateExecutionContext(
        ...
        long? take = null,
        string sourceContextId = "source")
    {
        var identity = new SourceIdentity("separatedvalues", "comma", sourceContextId, "Rows");
        ...
        var plan = schema.TryPlanSource(
            ...
            true,
            0).ExecutionPlan;
        var executionColumns = requiredColumns
            .Select((name, index) =>
            {
                var declared = columns.Single(column =>
                    string.Equals(column.ColumnName, name, StringComparison.Ordinal));
                return (ISchemaColumn)new SchemaColumn(name, index, declared.ColumnType);
            })
            .ToArray();
        return RuntimeV2TestContexts.CreateExecutionContext(
            allColumns: executionColumns,
            sourceRuntimeSettings: settings,
            executionPlan: plan);
    }
        ...
        SourceExecutionContext context)
    {
        ...
        if (legacy != structRows || legacy != classRows)
        {
            throw new InvalidOperationException(
                ...
                $"frozenLegacy={legacy}, struct={structRows}, class={classRows}.");
        }
    }
    ...
    private void VerifyFailureBehavior()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Query-row failure oracle failed: frozenLegacy={legacy}, " +
                $"struct={structRows}, class={classRows}.");
        }
    }
