...
public class OsAndGitToSqlTests
{
    ...
    private Task<UnpackedRepository> UnpackGitRepositoryAsync(string zippedRepositoryPath, int method, [CallerMemberName] string? testName = null)
    {
        if (testName is null)
        ...
        var repositoryPath = Path.Combine(directory, "Repositories", ".TestsExecutions", method.ToString(), testName);
    ...
    {
        return InstanceCreatorHelpers.CompileForExecution(script, Guid.NewGuid().ToString(), new OsAndGitSchemaProvider(),
