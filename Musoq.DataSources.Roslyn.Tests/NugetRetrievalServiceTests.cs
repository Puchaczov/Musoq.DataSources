...
<package xmlns=""http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd"">
  <metadata>
    ...
    public async Task GetMetadataFromPathAsync_WhenPropertyNotFound_ReturnsNull()
    {
        ...
        // Act
        var result =
            await _service.GetMetadataFromPathAsync(_commonResources, "NonExistentProperty", CancellationToken.None);
    ...
    public async Task GetMetadataFromNugetOrgAsync_WhenPropertyNotInStrategies_ReturnsNull()
    {
        ...
        // Act
        var result = await _service.GetMetadataFromNugetOrgAsync(baseUrl, _commonResources, "NonExistentProperty",
    ...
    public async Task GetMetadataFromNugetOrgAsync_WhenPropertyExists_ReturnsExpectedValue()
    {
        ...
        _httpClient.AddResponse(url,
            $"<html><body><a href=\"{licensePath}\" data-track=\"outbound-license-url\">License</a></body></html>");
        _httpClient.AddResponse(licenseUrl,
            $"<html><body><pre class=\"license-file-contents custom-license-container\">{expectedLicense}</pre></body></html>");
        ...
        // Act
        var result =
            await _service.GetMetadataFromNugetOrgAsync(baseUrl, _commonResources, "LicensesNames",
                CancellationToken.None);
    ...
    public async Task GetMetadataFromCustomApiAsync_WhenApiReturnsData_ReturnsExpectedValue()
    {
        ...
        var requestUrl =
            $"{apiEndpoint}?packageName={SamplePackageName}&packageVersion={SamplePackageVersion}&propertyName={propertyName}";
    ...
    public async Task GetMetadataFromCustomApiAsync_When404ErrorOccurs_ThrowsHttpRequestException()
    {
        // Arrange
        var requestUrl =
            $"https://api.example.com/nuget?packageName={SamplePackageName}&packageVersion={SamplePackageVersion}&propertyName=ProjectUrl";
    ...
    public async Task DownloadPackageAsync_WhenSuccessful_ReturnsPackagePath()
    {
        ...
        // Act
        var result = await _service.DownloadPackageAsync(SamplePackageName, SamplePackageVersion, SamplePackagePath,
            CancellationToken.None);
...
internal class FakeFileSystem : IFileSystem
{
    ...
    public Task WriteAllTextAsync(string path, string content, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrEmpty(directoryPath)) _directories.Add(directoryPath);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        foreach (var file in subDirectories.SelectMany(subDirectory =>
                     _files.Keys.Where(f => f.StartsWith(subDirectory) && subDirectory != directory)))
            yield return file;
...
internal class FakeHttpClient : IHttpClient
{
    ...
    public Task<TOut?> PostAsync<TOut>(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        if (!_responses.TryGetValue(
                request.RequestUri?.ToString() ?? throw new InvalidOperationException("Request URI is null"),
                out var response))
            return Task.FromResult(default(TOut));
