...
public class ExtractFromProjectMetadataTests
{
        ...
        public TestScenario ThenReturnRequestContentFromFile(string filePath)
        {
            if (_lastRequestUrl == null)
                throw new InvalidOperationException(
                    "WhenRequesting must be called before ThenReturnRequestContentFromFile");
    ...
    private class TestFileSystem : IFileSystem
    {
        ...
        {
            return _files.ContainsKey(path) || _externalFiles.Contains(path) || File.Exists(path);
        }

        public bool IsDirectoryExists(string path)
        {
            return _directories.Contains(path) || Directory.Exists(path);
        }
