...
public class AlwaysUpdateDirectoryViewTests
{
    ...
    public async Task MultipleAddAndRetrieve_ShouldSucceed()
    {
        ...
        // Act
        for (var i = 0; i < 5; i++) directoryView.Add($"key{i}", new TestItem($"Item{i}", i * 10));
    ...
    public async Task UpdateExistingItem_ShouldOverwriteValue()
    {
        ...
        {
            if (directoryView.TryGetValue("updateKey", out var item) && item is not null) return item.Name == "Updated";
            return false;
        });
    ...
    public async Task FileCreatedExternally_ShouldBeDetectedAndCached()
    {
        ...
        if (result != tcs.Task)
        {
            var scanMethod = typeof(AlwaysUpdateDirectoryView<string, TestItem>)
                .GetMethod("ScanDirectory", BindingFlags.Instance | BindingFlags.NonPublic);
        ...
        {
            var existingFiles =
                string.Join(", ", _mockFileSystem.GetFiles(_testDirectory, false, CancellationToken.None));
            var fileExists = _mockFileSystem.IsFileExists(filePath);
            var fileContents =
                fileExists ? await _mockFileSystem.ReadAllTextAsync(filePath, CancellationToken.None) : "N/A";
    ...
    public async Task FileRenamedExternally_ShouldUpdateCache()
    {
        ...
        await Task.WhenAny(Task.WhenAll(newKeyAddedCompletionSource.Task, oldKeyRemovedCompletionSource.Task),
    ...
    private static TestItem GetDestinationValue(string path, IFileSystem fileSystem,
    ...
    private static void UpdateDirectory(string path, TestItem value, IFileSystem fileSystem,
        CancellationToken cancellationToken)
    {
