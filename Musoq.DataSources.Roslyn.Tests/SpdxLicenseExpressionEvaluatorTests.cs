...
public class SpdxLicenseExpressionEvaluatorTests
{
    ...
    public async Task SimpleOperators_ReturnCorrectIdentifiers(string? expression, string[] expected)
    {
        ...
        foreach (var id in expected) CollectionAssert.Contains(result, id);
    }
    ...
    [TestMethod]
    [DataRow("(GPL-3.0-only WITH Autoconf-exception-3.0) OR MIT",
        new[] { "GPL-3.0-only", "Autoconf-exception-3.0", "MIT" })]
    [DataRow("MIT AND (Apache-2.0 OR (BSD-3-Clause WITH GCC-exception-3.1))",
        new[] { "MIT", "Apache-2.0", "BSD-3-Clause", "GCC-exception-3.1" })]
    [DataRow("(LGPL-2.1+ WITH Unix-exception-2.0) AND (GPL-2.0 OR MIT)",
        new[] { "LGPL-2.1+", "Unix-exception-2.0", "GPL-2.0", "MIT" })]
